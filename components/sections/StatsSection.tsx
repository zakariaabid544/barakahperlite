...
export function StatsSection({ stats }: StatsSectionProps) {
  return (
    <section className="relative z-10 -mt-8 px-4 pb-12 sm:px-6 md:-mt-10 md:pb-16 lg:px-8 lg:pb-20">
      <div className="mx-auto grid max-w-7xl grid-cols-2 items-stretch gap-2 rounded-[0.45rem] border border-white/10 bg-basalt-900/80 p-2 shadow-glass backdrop-blur-2xl md:grid-cols-4 md:gap-3 md:p-3">
        {stats.map((stat, index) => (
          <Reveal
            key={stat.label}
            delay={index * 0.05}
            className="rounded-[0.35rem] border border-white/10 bg-white/[0.045] p-4 md:p-5"
          >
